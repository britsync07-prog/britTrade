...
function formatDCAMessage(strategyName, signal) {
  const { symbol, side, price, tp, sl, stakeAmount } = signal;
  const isLong = side === 'buy' || side === 'long';
  const sideEmoji = '📉'; // Averaging down implies buying dips or shorting peaks
  const sideLabel = isLong ? '📈 BUY (DCA)' : '📉 SELL (DCA)';

  return (
    `${sideEmoji} *DCA ENTRY (Averaging Down) — ${strategyName}*\n` +
    `━━━━━━━━━━━━━━━━━━━━━━\n` +
    `*Pair:*         \`${symbol}\`\n` +
    `*Direction:*    ${sideLabel}\n` +
    `*DCA Price:*    \`$${formatPrice(price)}\`\n` +
    `*New TP:*       \`$${formatPrice(tp)}\` 🎯\n` +
    `*New SL:*       \`$${formatPrice(sl)}\` 🛑\n` +
    `*Stake:*        \`$${stakeAmount || 10}\`\n` +
    `━━━━━━━━━━━━━━━━━━━━━━\n` +
    `_Signal generated by BritSyncAI_`
  );
}
...
class TelegramService {
  ...
  async broadcastSignal(signal) {
    ...
    let message;
    if (isEntry) {
      message = signal.isDCA ? formatDCAMessage(strategy.name, signal) : formatEntryMessage(strategy.name, signal);
    } else {
      message = formatExitMessage(strategy.name, signal);
    }
